...
Titanium.API.info(tit);
var value = Titanium.App.Properties.getString(tit);

var order = new zamowienie();
order = Titanium.JSON.parse(value);
 Titanium.API.info("xxx"+value+  " "+order.nazwa+  " "+order.numer+  " "+order.date_start +" " + order.date_stop);  
    if (order !== null) {
        ...
        zapisz_bt.addEventListener('click', function(){
            if (flaga == 0) {
				...
				Titanium.API.info("order sklad w edicie "+order.nazwa + " "+ order.numer + " "+order.date_start + " " + order.date_stop);
								
				//Titanium.App.Properties.removeProperty(tit);
